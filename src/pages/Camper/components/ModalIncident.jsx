...
export default function ModalIncident({
    ...
    const GetClassroom = async () => {
        ...
        try {
            const sever = JSON.parse(import.meta.env.VITE_MY_SERVER);
            const response = await (await fetch(`http://${sever.host}:${sever.port}/zones/Classroom?nameArea=${Area}`, options)).json();
            if (response.status === 200) {
    ...
    const GetInventory = async () => {
        ...
        try {
            const sever = JSON.parse(import.meta.env.VITE_MY_SERVER);
            const response = await (await fetch(`http://${sever.host}:${sever.port}/inventory?zoneId=${Zone_id}`, options)).json();
            if (response.status === 200) {
    ...
    const submitReportIncident = async(e) =>{
        ...
        let options = {
          ...
          try {
            const sever =JSON.parse(import.meta.env.VITE_MY_SERVER);
            const response = await (await fetch(`http://${sever.host}:${sever.port}/incidents`, options)).json();
            if(response.status === 200){
              alert(response.message)
              setShow(false)
            } else{
                ...
                <div className='row fs-5'>
                    ...
                    <div className=" col-12 col-md-6 mb-3">
                        ...
                        <select onChange={(e) => {
                            ...
                            {
                                Zonas?.map(item=><option key={item.Area} value={item.Area}>{item.Area}</option> )
                            }
                        </select>
                    </div>
                    <div className=" col-12 col-md-6 mb-3">
                        ...
                        <select onChange={(e) => {
                            ...
                            {
                            Classrooms?.map(item=><option key={item.ID} value={item.ID}>{item.Classroom}</option> )
                            }
                        </select>
                    </div>
                    <div className=" col-12 col-md-6 mb-3">
                        <label className="form-label">Inventario:</label>
                        <select value={Inventory_id} onChange={(e) => {
                            const newId = parseInt(e.target.value)
                            setInventory_id(newId)
                        }} className="form-select" aria-label="Default select example">
                            <option  value="">Otro</option>
                            {
                             (Inventarios != []) &&  Inventarios[0]?.Inventory_Info.map(item=><option key={item.ID} value={item.ID}>{item.Object.Name_Object}</option> )
                            }
                        </select>
                        </div>
                    ...
                    <div className='col-12 containerLoginHeader mt-4'>
                        <ButtonLogin type="submit" name="Agregar" styles="btn btn-primary ms-2 fs-6 buttonSignup" />
                        <button className="btn btn-danger text-black fs-6 text-white ms-2 mt-3 mt-md-0 buttonLogin" onClick={() => setShow(!show)}>Cerrar</button>
                    </div>
                </div>
            </form>
        </div>
