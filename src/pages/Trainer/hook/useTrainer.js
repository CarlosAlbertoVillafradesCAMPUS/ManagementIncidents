...
const useTrainer = () => {
    ...
    const CampersAll = async() => {
        ...
        let options = {
          ...
          try {
            const sever =JSON.parse(import.meta.env.VITE_MY_SERVER);
            const response = await (await fetch(`http://${sever.host}:${sever.port}/users?rol=Camper`, options)).json();
            if(response.status === 200){
    ...
    const GetPending = async() => {
        ...
        let options = {
          ...
          try {
            const sever =JSON.parse(import.meta.env.VITE_MY_SERVER);
            const response = await (await fetch(`http://${sever.host}:${sever.port}/incidents/ordenados?status=Pending`, options)).json();
            if(response.status === 200){
                let newArray=[];
                response.data.forEach((val,id) => {
                    newArray.push({
                        By_Camper: {Nit: val.By_Camper.Nit, Nickname: val.By_Camper.Nickname},
    ...
    const GetSolved = async() => {
      ...
      let options = {
        ...
        try {
          const sever =JSON.parse(import.meta.env.VITE_MY_SERVER);
          const response = await (await fetch(`http://${sever.host}:${sever.port}/incidents/ordenados?status=Solved&rol=Trainer&nit=${stateTrainer.Nit}`, options)).json();
          if(response.status === 200){
    ...
    const GetAssign= async() => {
        ...
        let options = {
          ...
          try {
            const sever =JSON.parse(import.meta.env.VITE_MY_SERVER);
            const response = await (await fetch(`http://${sever.host}:${sever.port}/incidents/ordenados?status=Assigned&rol=Trainer&nit=${stateTrainer.Nit}`, options)).json();
            if(response.status === 200){
